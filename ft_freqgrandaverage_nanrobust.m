function [grandavg] = ft_freqgrandaverage_nanrobust(cfg, varargin)

% FT_FREQGRANDAVERAGE_NANROBUST — same as FieldTrip FT_FREQGRANDAVERAGE, but the
% grand mean over subjects uses MEAN(...,'omitnan') so NaNs (e.g. failed FOOOF
% bins in one subject) do not poison the average for all subjects.
%
% Use as
%   [grandavg] = ft_freqgrandaverage_nanrobust(cfg, freq1, freq2, freq3...)
%
...
%
% See also FT_FREQGRANDAVERAGE, FT_TIMELOCKGRANDAVERAGE, FT_FREQANALYSIS,
% FT_FREQDESCRIPTIVES, FT_FREQBASELINE
...
for k=1:numel(cfg.parameter)
  if strcmp(cfg.keepindividual, 'no')
    dat_first = varargin{1}.(cfg.parameter{k});
    nd = ndims(dat_first);
    C = cell(1, Nsubj);
    for s=1:Nsubj
      C{s} = varargin{s}.(cfg.parameter{k});
    end
    stack = cat(nd + 1, C{:});
    tmp = mean(stack, nd + 1, 'omitnan');
